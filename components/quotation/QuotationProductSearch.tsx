...
export function QuotationProductSearch({ products, categories, onAdd }: QuotationProductSearchProps) {
        ...
        {filtered.map(p => {
          ...
          return (
            <button
              key={p.id}
              onClick={() => onAdd(p)}
              className="w-full text-left p-3 rounded-lg border border-gray-200 hover:border-emerald-500 hover:shadow-md transition-all"
            >
              <div className="flex justify-between items-start mb-1">
                <span className="font-medium text-gray-900">{p.name}</span>
                <span className="text-emerald-700 font-bold">₹{finalPrice.toFixed(2)}</span>
              </div>
